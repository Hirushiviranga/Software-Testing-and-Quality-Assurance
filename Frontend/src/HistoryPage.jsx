...
export default function HistoryPage() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/CalculatorHistory`, {
  ...
  const deleteCalc = async (id) => {
    await fetch(`${BASE_URL}/CalculatorHistory/${id}`, {
      method: "DELETE",
      headers: { "X-API-KEY": API_KEY } // ✅ Added
    });
    setHistory(history.filter(h => h.id !== id));
  ...
  const saveEdit = async (id) => {
    ...
    const newExpression = `${editCalc.num1} ${
      editCalc.operation === "add" ? "+" :
      editCalc.operation === "subtract" ? "-" :
      editCalc.operation === "multiply" ? "*" : "/"
    } ${editCalc.num2}`;
